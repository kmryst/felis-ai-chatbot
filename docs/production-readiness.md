...
| `/chat` の公開面 | 恒久構成（Easy Auth + BFF + backend internal ingress）を **Terraform / authConfigs まで実装済み**（Issue #194: frontend Container App + `authConfigs`（AzAPI）+ `backend_ingress_external` 切替スイッチ + `CHAT_API_KEY_CONFIG_CHECKSUM` 両側適用。bootstrap は fail-closed 順序 = [vnet-integration-cutover.md §7](./operations/vnet-integration-cutover.md)。デプロイ・cutover の実測記録は `docs/verification/` の frontend-easy-auth-cutover 配下）。レート制限（認証後段の route dependency）は未実装 | 常時稼働への転換（ADR-0020）で 24/7 露出になるため認証と緊急遮断を先に入れ、恒久構成は実測（#183 / #184）反映後に execution plan の作業順で実装した | Easy Auth（Entra ID）+ BFF + backend internal ingress + 認証後段のレート制限（決定 = ADR-0027） | [ADR-0027](./adr/0027-frontend-azure-deployment-and-public-surface.md) / **Issue #107** / **Issue #113** |
