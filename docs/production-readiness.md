...
- **主成果物は PostgreSQL の Backup / PITR / Maintenance / Monitoring / HA の設計・実施・記録**（[day3-5-execution-plan.md §0](./operations/day3-5-execution-plan.md)）であり、**それ以外は意図的にスコープ外**にしている
...
| [credit-window-execution-plan.md](./operations/credit-window-execution-plan.md) | これから何をやるか（Day 4 以降の正本） |
| [day3-5-execution-plan.md](./operations/day3-5-execution-plan.md) | Day 3 の記録と、§2 事実表 / §7〜§9 の正本（§4 / §5 は credit-window plan に superseded） |
...
| `/chat` の公開面 | 恒久構成（Easy Auth + BFF + backend internal ingress）を **Terraform / authConfigs まで実装済み**（Issue #194: frontend Container App + `authConfigs`（AzAPI）+ `backend_ingress_external` 切替スイッチ + `CHAT_API_KEY_CONFIG_CHECKSUM` 両側適用。bootstrap は fail-closed 順序 = [vnet-integration-cutover.md §7](./operations/vnet-integration-cutover.md)。デプロイ・cutover の[実測記録](./verification/frontend-easy-auth-cutover/observations.md)）。レート制限（認証後段の route dependency）は未実装 | 常時稼働への転換（ADR-0020）で 24/7 露出になるため認証と緊急遮断を先に入れ、恒久構成は実測（#183 / #184）反映後に execution plan の作業順で実装した | Easy Auth（Entra ID）+ BFF + backend internal ingress + 認証後段のレート制限（決定 = ADR-0027） | [ADR-0027](./adr/0027-frontend-azure-deployment-and-public-surface.md) / Issue #107（CLOSED） / **Issue #113** / **Issue #208** |
...
| 項目 | 現状 | なぜ現状こうなのか | 本番なら | 追跡先 |
| --- | --- | --- | --- | --- |
| HA / 冗長構成 | 常設では無し。イベント計測として **2026-08-28 に約 2.5 時間で実施済み**（tier 昇格 → HA 有効化 → planned / forced failover → HA 無効化 → tier 復帰。planned 23.9 秒 / forced 30.4 秒。終了後は B1ms・HA `Disabled` へ復帰済み = [実測記録](./verification/failover-drill/observations.md)）。HA 有効時の drift を Terraform が検出しない状態（`ignore_changes`）の扱いは未決 | HA（GP ×2 台）の常設は課金が大きく、検証目的はフェイルオーバーの実測で足りる | 要件に応じたゾーン冗長 HA の常設 | [実測記録](./verification/failover-drill/observations.md) / [ADR-0023](./adr/0023-no-second-granularity-downtime-measurement.md) / **Issue #157**（HA 状態の扱いの確定） |
| geo 冗長バックアップ | 有効（2026-08-22 の cutover ステップ A の再作成で実機反映済み。`geoRedundantBackup: Enabled` を実測 = [vnet-cutover 記録](./verification/vnet-cutover/observations.md)）。ただし geo リストアの手順整備・演習は未実施 | 無料枠の判明で 2 倍課金の前提が崩れ、作成時のみ設定可の制約を cutover の再作成で解消した。geo リストアは PITR 不可・RPO 最大 1 時間で主成果物に寄与しないため演習はスコープ外 | DR 要件に基づく geo リストア手順（ペアリージョン側 VNet 含む）の整備と定期演習 | [ADR-0019](./adr/0019-enable-geo-redundant-backup.md) / [ADR-0011](./adr/0011-backup-retention-and-geo-redundancy.md) |
...
| 項目 | 現状 | なぜ現状こうなのか | 本番なら | 追跡先 |
| --- | --- | --- | --- | --- |
| 監視・アラート | メトリックアラート 5 件（`is_db_alive` / storage free low・critical / CPU credits remaining low / storage percent 80）+ Action Group（メール通知）を persistent 層の Terraform で管理。2026-08-28 のドリル中に Sev0 `alert-pgsql-is-db-alive` の実発火と自動 Resolve まで実測済み。**初動 Runbook は無し** | Monitoring は当初「時間不足なら削る筆頭」だったが、常時稼働への転換（ADR-0020）で無人時間帯が生まれたため導入し、Terraform 管理下へ import した | メトリックアラート + 通知経路 + **初動 Runbook**（Runbook が本番差分として残る） | [ADR-0022](./adr/0022-import-azure-monitor-into-terraform.md) / [台帳](./operations/azure-resource-inventory.md) / [発火の実測](./verification/failover-drill/observations.md) |
| コスト監視 | 手動（CLI での残高・リソース確認を終業時に実施）。**Budget アラート導入は計画済み・実装待ち**（総消費上限 150 USD に対し 4 段閾値 75.00 / 112.50 / 135.00 + 予測 150。2026-08-30 更新） | 当初 5 日間の検証では日次の手動見張りで足りる判断だった。常時稼働への転換（ADR-0020）で無人時間帯が生まれるため自動検知を導入する | Budget + コストアラートによる自動検知 | **Issue #105** / [credit-window-execution-plan.md](./operations/credit-window-execution-plan.md) §4 / §8 |
...
| Capacity planning | 計画としては無し。あるのは実測のみ（フェーズ 1 の低負荷ベースライン: WAL 生成レート・DB サイズ増加・接続数・ストレージ使用率。**フェーズ 2 の高負荷比較は未実施** = Issue #112） | 需要予測の元になる実トラフィックが存在せず、フェーズ 1 の書き込みは PITR の復旧時点を確定させる recovery marker（毎分 1 行 + カウンタ 1 行 UPDATE）であって負荷生成ではない（[ADR-0021](./adr/0021-heartbeat-table-as-recovery-marker.md)）。この数字を「容量計画」と呼ぶと崩れる（同じ理由で監視閾値も「決めた」とは書いていない） | 需要予測とヘッドルーム方針、自然増・非自然増の分離、それに基づくスケールとコストの判断 | [credit-window-execution-plan.md §5-5](./operations/credit-window-execution-plan.md) / **Issue #112**（フェーズ 2 の負荷観測まで。予測は範囲外） |
...
| リストア試験の継続性 | **PITR ドリルは未実施**（1 回目・2 回目とも。2026-09-02〜09-15 の作業窓で単発実測を予定。定期・自動化の計画は無し） | 目的が「設計・実行した記録」であり、継続運用のフェーズがない。実施自体が主成果物の中心で、まだ取れていない | 定期リストア試験のスケジュール化と演習記録 | [credit-window-execution-plan.md §7 / §10-6](./operations/credit-window-execution-plan.md) / [復旧目標の宣言](./operations/restore-drill-recovery-objectives.md) |
...
| 管理外リソース（[台帳 §B](./operations/azure-resource-inventory.md) の一覧が正本） | OIDC アプリ / RG / tfstate Storage / マネージド ID / ロール割当 / Easy Auth 用アプリ登録・service principal・テストユーザー等が手動管理 | 鶏と卵（Terraform の足場）・権限の器・据え置き判断のいずれかで、台帳 + 読み取り確認コマンドを差分検出の代替とした | bootstrap 層も別 state で IaC 化するか、台帳運用を継続的に検証する | [台帳 §B](./operations/azure-resource-inventory.md) / [ADR-0012](./adr/0012-least-privilege-oidc-sp-and-dedicated-terraform-rg.md) / [ADR-0015](./adr/0015-ephemeral-layer-acr-container-apps-design.md) |
...
| フロントエンドの配信 | Container Apps への frontend デプロイ（`ca-felisaichatbot-dev-front`。Next.js standalone image + Easy Auth）を **Terraform で実装済み**（Issue #194。`min_replicas = 1` = ADR-0027 決定 9、イメージは 3 本目として `DEPLOY_SHA` 共有 = 決定 7。デプロイの[実測記録](./verification/frontend-easy-auth-cutover/observations.md)） | 恒久構成が ADR-0027 で決定済みになり、実測（#183）反映後に実装した | Container Apps への frontend デプロイと認証つき公開面（この構成そのもの。静的ホスティング / CDN 案は ADR-0027 で却下） | **Issue #194** / [frontend-sse-execution-plan.md](./operations/frontend-sse-execution-plan.md) / [ADR-0027](./adr/0027-frontend-azure-deployment-and-public-surface.md) |
| RAG データの投入経路 | seed 投入（気象庁シードの diff-sync）と実 embedding backfill を Container App Job 2 本（`caj-felisaichatbot-dev-seed` / `caj-felisaichatbot-dev-embed`。Manual トリガー）として **Terraform で実装済み**（Issue #196。手順の正本は [seed-and-embedding-backfill.md](./operations/seed-and-embedding-backfill.md)、実行の[実測記録](./verification/seed-embedding-backfill/observations.md)）。実行は手動起動のみで、データ更新の定期実行はない | 題材データは固定シード（気象庁ページ由来）で更新頻度の要件がなく、destructive な seed 投入と再実行安全な backfill をリソース分離した手動 Job が最小の実行手段だった | データ更新パイプライン（取得〜チャンク〜embedding の自動化・スケジュール実行）と、データ追加時の類似度閾値再測定の運用化 | **Issue #196** / [ADR-0010](./adr/0010-rag-wiring-and-hallucination-guard.md) |
| 利用者の範囲 | アプリのロール `Chat.Use` を割り当てているのは所有者 1 アカウントのみ。`appRoleAssignmentRequired = true` のため、URL を共有しても所有者以外は `AADSTS50105` で拒否され、**第三者は動作を確認できない** | LLM 課金を伴うエンドポイントを匿名公開しない方針（ADR-0027 の決定）を維持したまま最短で認証面を閉じたため、利用者の範囲を広げる設計は後回しにした | 想定利用者の範囲と提供期間を定義したうえでの割当方式（招待 / グループ / セルフサービス）と、認証後段のレート制限 | **Issue #208** / [ADR-0027](./adr/0027-frontend-azure-deployment-and-public-surface.md) / Issue #113 |
