...
| 1 | `terraform -chdir=terraform/persistent apply`（PostgreSQL + Log Analytics + Key Vault の secret / log search alert。Entra 認証のみで作成され `felisadmin` は存在しない = ADR-0031） | 約 7 分（2026-08-21 実測。サーバー本体 5m32s。[restore-drill/observations.md](../verification/restore-drill/observations.md)） |
