...
- SLI specification は確定済み（ADR-0028 決定 11。PR #241）。SLI implementation の方式は authenticated synthetic transaction に
  決定済み。timestamp field、event time、latency clock は確定済みで、その他の configuration
...
| SLI implementation の timestamp field、event time、latency clock を定義する | critical user journey、measurement point、compliance period | wall clock field、SLI の event time、window boundary、latency 用 monotonic clock の取得位置を定義する | version 管理された timestamp field と clock の定義 | supported-client path、clock と window boundary の test case | event の期間帰属または latency を同じ raw record から再現できなければ停止する | 完了（slo-document.md「Timestamp field」以下 3 小節） |
| SLI implementation を実装し検証する | 選んだ方式と timestamp field / event time / latency clock の定義 | その定義に従って payload、identity、location、schedule、measurement timeout を決め、下記「SLI implementation の検証」を通す | schema、tool、version、validation evidence | prototype record と field 単位の coverage | 必須 case が誤分類されるか、暗黙に失われるなら有効化を停止する | 未着手 |
...
### Timestamp field、event time、latency clock を検証する
...
各 path について、期待する eligibility と outcome、観測した field、実際の classification、`slo-document.md` の timestamp field の
定義に従う時刻、monotonic elapsed time、raw record を保存する。
