...
authenticated synthetic transaction の記録項目とデータ形式を定義する。
...
- SLI specification、eligible / good / bad event、SLO、timestamp fields / event time / clock の定義は
  [slo-document.md](./slo-document.md) を正本とする。
- 実装の選定・検証、baseline、SLO の採用・review は [slo-review-runbook.md](./slo-review-runbook.md) を正本とする。
...
[slo-document.md の Timestamp fields](./slo-document.md#timestamp-fields) を参照する。ここでは項目一覧と欠落時の扱いを示す。
...
[既存の clock 定義](./slo-document.md#monotonic-clock-による-latency-計測)に従う。
...
分類の定義は [SLI と SLO](./slo-document.md#sli-と-sloslis-and-slos) と runbook を参照する。
