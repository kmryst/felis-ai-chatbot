...
- フェーズ 1 終了見込み = T_obs_start + 72h = **2026-08-26T08:16Z 頃**
- 対象コミット: `main` = `56af4ae` / イメージ `sha-56af4ae` / alembic `0003`

> **フェーズ 1 は本記録の時点で進行中**（本追記の取得時刻 2026-08-25T05:32Z の時点で経過 約 45.3h / 72h）。
> 本ファイルの数値は**すべて記載の取得時刻における中間値**であり、
> 稼働率・レイテンシ・run 数などの最終値は**フェーズ 1 完走（2026-08-26T08:16Z）後に別途確定させる**。
> 2026-08-25 の追記は §2-3 の「通知先への到着記録」と §3-5（`gh run view --log` の欠陥）で、
> いずれも**失うと再取得できない証跡の保全**が目的。追記にあたり Azure・GitHub Actions・
> `obs.phase_config` のいずれにも書き込みを行っていない（読み取りのみ）。
...
| 6 | §5-3: 鮮度ゲートは probe 5 分間隔で「即日検知」 | **scheduled run は 22.5 時間で 53 回（期待 270 回の 19.6%）、最大間隔 98.8 分**（§4-5） | 検知遅延が最悪 1.6 時間。SLI の点数も設計の約 1/5。原因は外部レビュー中のため**本セッションでは条件を変えていない** |
