...
- 日次 Full backup の `completedTime` は毎朝 07:2xZ で、08-30 の 07:22:48 から 09-05 の 07:28:18 まで
  6 日で約 5 分 30 秒ずつ後ろへずれている（1 日あたり約 55 秒）
- 元サーバー `pgsql-felisaichatbot-dev` は 2 回のドリルを通じて無傷（破壊的操作なし）。
  全復元完了後の 2026-09-05T12:12:52.846Z に元サーバーで digest を再取得し、
  手順 0 の固定ベースラインと 4 テーブルすべて一致することを確認した
