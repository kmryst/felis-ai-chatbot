...
### 付記（2026-08-23。#103 外部レビューによる自己矛盾の検出）

本節で「min 0 でもレプリカは落ちない」を確定させた**同じ日**に書いた実行計画
（credit-window-execution-plan.md の初版 §9）が、打ち切りの最終手段として「ops min 0 化 →
常駐停止」を採用しており、本節の実測と正面から矛盾していた（外部レビューが検出、同 PR 内で
「証跡退避 → ephemeral destroy → 実体確認」へ修正済み）。**同日に確定した実測でも、別文書へは
書いた瞬間に照合しないと反映漏れが起きる**、という実例として残す。
