...
- ephemeral 層の毎日 destroy（時間課金を止める運用）は変えない（注記: この前提はその後 2 段階で変更された。private access 化で夜間 destroy をやめ = [ADR-0018 追記](./0018-postgresql-private-access-and-vnet-integration.md)、さらに [ADR-0020](./0020-credit-window-resource-strategy.md) の常時稼働方針で最終 destroy は [credit-window-execution-plan.md](../operations/credit-window-execution-plan.md) §9（2026-09-15 目安（当初 2026-09-16 想定 → フェーズ 1 の 72h 化で 2026-09-03〜09-04 へ前倒し → 2026-08-30 の作業窓再設定（8/27〜9/14）で 9/15 に確定 = 計画 §10-5））に確定）
...
- プロジェクト終了時の destroy は不変。本 ADR は「日々の運用」の話であり、「プロジェクト終了時」の話ではない（時間軸の区別は台帳 §A。注記: 終了時 destroy の時期は当初「Day 5（計画書 §5-6）」→ [ADR-0020](./0020-credit-window-resource-strategy.md) により失効前 teardown = 新計画 §9 の 2026-09-15 目安（当初 2026-09-16 想定 → フェーズ 1 の 72h 化で 2026-09-03〜09-04 へ前倒し → 2026-08-30 の作業窓再設定（8/27〜9/14）で 9/15 に確定 = 計画 §10-5） へ変更。「本 ADR の stop しない判断」自体は ADR-0020 の常時稼働方針と整合し、むしろ前提として強化された）
