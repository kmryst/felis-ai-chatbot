...
| [0030](./0030-subscription-migration-and-02-suffix-naming.md) | Accepted | 検証環境を新しい Azure サブスクリプションへ移行する（グローバル一意名 4 件は `02` サフィックス = ADR-0013 の例外。移行元は destroy せず残す。秘密値は全て新規発行。OIDC SP は作らない。DB 初期化は `upgrade 0001` → obs 復元 → `stamp head`） |
| [0031](./0031-entra-managed-identity-auth-and-remaining-secrets.md) | Accepted | PostgreSQL と Azure OpenAI の認証を managed identity の Microsoft Entra 認証へ移し、パスワード / API キー認証を無効化する。残す秘密値は Easy Auth のクライアントシークレット（implicit flow を避けるため）と Terraform 生成の chat API キー（無認証公開の砦）の 2 つ。DB ロールは `GRANT felisadmin` で管理者権限を継承（最小権限化は #86 / #280）。段階的に切り替え、各段階の再起動・拒否応答を実測 |
| [0032](./0032-key-vault-references-for-container-apps-secrets.md) | Proposed | Easy Auth のクライアントシークレットと chat API キーを Azure Key Vault（RBAC・persistent 層・soft-delete 7 日・purge protection 無効）に置き、Container Apps の secret をバージョン無しの Key Vault 参照にして tfvars と Terraform state から値を外す。ロール割当は手動（ADR-0012）、同期失敗は log search alert で検知する |
